...
comparing compound total areas against internal standard total areas.
"""
...
def test_peak_area_validation_math():
    ...
    assert is_valid(compound_total=20.0, internal_standard_total=200.0, ratio=0.05)
    
    # Test failing threshold: 5.0 < 0.05 * 200.0 (10.0)
    assert not is_valid(compound_total=5.0, internal_standard_total=200.0, ratio=0.05)
    
    # Test edge case: exactly at threshold
    assert is_valid(compound_total=10.0, internal_standard_total=200.0, ratio=0.05)
    
    # Test with different ratio
    assert is_valid(compound_total=15.0, internal_standard_total=200.0, ratio=0.05)
    assert not is_valid(compound_total=8.0, internal_standard_total=200.0, ratio=0.05)


def test_data_provider_peak_metrics_totals():
    """DataProvider should expose compound and internal standard total areas."""
    ...
    assert metrics["CompoundA"]["internal_standard_total"] == pytest.approx(55.0)
    
    # Verify ISTD totals
    assert metrics["ISTD"]["compound_total"] == pytest.approx(55.0)
    assert metrics["ISTD"]["internal_standard_total"] == pytest.approx(55.0)
...
def test_data_provider_peak_metrics_with_session_overrides():
    ...
    assert metrics["CompoundB"]["internal_standard_total"] == pytest.approx(500.0)
