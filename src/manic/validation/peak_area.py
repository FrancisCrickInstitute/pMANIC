...
def is_valid(compound_total: float, internal_standard_total: float, ratio: float) -> bool:
    """
    Check if a compound's total area meets the minimum threshold.
    
    This is a pure mathematical function that compares:
        compound_total >= (internal_standard_total × ratio)
    
    Args:
        compound_total: Sum of all isotopologue areas for the compound
        internal_standard_total: Sum of all isotopologue areas for the internal standard
    ...
    """
    if internal_standard_total <= 0:
        return True
    
    threshold = internal_standard_total * ratio
