...
def write(workbook, exporter, progress_callback, start_progress: int, end_progress: int, *, provider=None, validation_data=None) -> None:
    ...
    for sample_idx, sample_name in enumerate(samples):
        ...
        for compound_row in compounds:
            ...
            for isotope_idx in range(num_isotopologues):
                if isotope_idx < len(ratios):
                    ratio_value = ratios[isotope_idx]
                else:
                    ratio_value = None if label_atoms > 0 else 0.0
                ...
                worksheet.write(row, col, ratio_value, fmt)
