...
Qualifier ions are secondary fragments used to **support identity**.

Because fragmentation is characteristic, the ratio

\[
\frac{\text{area of qualifier ion}}{\text{area of Q ion}}
\]
...
MANIC accepts one or two qualifier ions. Old MANIC required two; pythonMANIC requires at
least one.
...
### Why labelled mode does not use qualifier ions
...
itself. Applying Q/qualifier ratio logic there would confuse biology with identity QC.
...
- If every non-empty Q and qualifier ion in that sample fitted, amount and qualifier/Q both use the
  model areas. Amount is still the Q-ion area alone.
- If any non-empty Q or qualifier ion failed to fit, every non-empty ion of that
  compound/sample uses the raw in-window scans for amount and qualifier/Q. Empty ions
  stay at area 0. That keeps qualifier/Q as one kind of measurement (never a model area
...
Imported expected qualifier/Q ratios are almost always measured on raw-window areas.
...
Natural-isotope correction is not applied. Q and qualifier ions are diagnostic ions, not
...
- Exactly one Q ion and at least one qualifier ion
...
1. Measure qualifier/Q area ratios across several clean injections of standards or
...
Tile borders mark a missing Q peak (grey). Qualifier/Q review stays on the Identity
...
isotope row and no qualifier-ion columns. `Mass` is the Q *m/z*.
...
(Peak Area / nmol / Relative), qualifier-ion raw areas sit on Qualifier QC,
...
Trace colours follow the shared channel palette (Q is the first colour; qualifier 1 is
typically the second). Observed Q apex is used for the mass spectrum and
export; it is not drawn on the tiles.

If deconvolution is on, each Q or qualifier ion that fitted is drawn as a smooth
...
The channel legend above the grid names each Q/qualifier *m/z*.
