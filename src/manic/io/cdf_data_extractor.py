...
from manic.io.tic_reader import TICData
from manic.io.ms_reader import MSData
...
def extract_ms_on_demand(
    ...
    try:
        ...
        if len(mz_values) == 0:
            logger.warning(
                f"No MS data extracted for {sample_name} at {retention_time:.3f} min"
            )
            return None
        ...
        if store_ms_data(sample_name, actual_time, mz_values, intensities):
            logger.info(
                f"Stored MS data for {sample_name} at {actual_time:.3f} min: {len(mz_values)} peaks"
            )
            # Return the newly extracted data
...
def _extract_ms_at_time_from_cdf_data(
    cdf_data: CdfFileData, target_time: float, tolerance: float = 0.1
